...
const Index: React.FC = () => {
  ...
  const handleClickItem = (id: any) => {
    ...
    if (conversation && conversation.messages && conversation.messages.length > 0 && conversation.users) {
  ...
  const renderConversation = useCallback(() => {
    return (
      ...
      chat.conversations.map((conversation) => (
        <ItemConversation key={conversation.id} conversation={conversation} handleClick={handleClickItem} />
      ))
    );
  }, [chat.conversations, handleClickItem]);
  ...
  const renderContentConversation = useCallback(() => {
    ...
    if (conversationActive?.messages) {
      return (
        ...
        SortData(conversationActive.messages).map((message, index) => {
  ...
  return !chat || chat.loading ? (
    ...
    <div className={classes.parentConversation}>
      <div className={classes.conversations}>{renderConversation()}</div>

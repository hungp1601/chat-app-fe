...
const Index: React.FC<Props> = (props) => {
    ...
    if (conversation.users.length >= 2) {
      return (
        <Badge
          ...
          badgeContent={<Avatar className={classes.smallAvatar} alt="Remy Sharp" src={Anh10} />}
        >
  ...
  const renderText = useCallback(() => {
    ...
    return <ListItemText primary={props.conversation.title} secondary={props.conversation.description} />;
  ...
  return (
    <List onClick={handleExpandClick}>
      <ListItem selected={conversation.active} button className={classes.listItem}>
        {renderAvatar()}
        {renderText()}
      </ListItem>
    </List>
